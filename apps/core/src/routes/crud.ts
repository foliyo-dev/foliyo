...
export function skillsRoutes(db: FoliyoDb) {
  return crudRoutes(db, "skills", z.object({
    name: z.string().min(1),
...
export function projectsRoutes(db: FoliyoDb) {
  return crudRoutes(db, "projects", z.object({
...
export function experienceRoutes(db: FoliyoDb) {
  return crudRoutes(db, "experience", z.object({
...
export function educationRoutes(db: FoliyoDb) {
  return crudRoutes(db, "education", z.object({
...
export function certificationsRoutes(db: FoliyoDb) {
  return crudRoutes(db, "certifications", z.object({
    name: z.string().min(1),
...
function crudRoutes(db: FoliyoDb, table: string, schema: z.ZodObject<z.ZodRawShape>) {
  ...
  r.get("/", (c) => {
    ...
    const items = queryAll(db, `SELECT * FROM ${table} WHERE user_id = ? ORDER BY sort_order`, [userId]);
    return c.json(items);
  });
  ...
  r.post("/", async (c) => {
    ...
    const values = [...Object.values(body.data), userId];
    run(db, `INSERT INTO ${table} (${cols.join(", ")}, user_id) VALUES (${placeholders}, ?)`, values);
    const items = queryAll(db, `SELECT * FROM ${table} WHERE user_id = ? ORDER BY sort_order`, [userId]);
  ...
  r.put("/:id", async (c) => {
    ...
    const values = [...Object.values(body.data), id, userId];
    run(db, `UPDATE ${table} SET ${sets}, updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?`, values);
  ...
  r.delete("/:id", (c) => {
    const userId = c.get("userId");
    const id = c.req.param("id");
    run(db, `DELETE FROM ${table} WHERE id=? AND user_id=?`, [id, userId]);
