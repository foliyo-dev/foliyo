...
1. Bump `version` in `apps/core` + `packages/{brand,ui,themes}` (and root) together.
2. Push to `main`, then tag: `git tag v0.5.0 && git push origin v0.5.0`
3. [Release workflow](../.github/workflows/release.yml) attaches self-host bundles **and** the four library tarballs.
